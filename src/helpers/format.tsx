    ...
    const processInline = (text: string): React.ReactNode[] => {
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            if (match[1])
                ...
                const href = /^https?:\/\//.test(full) ? full : 'http://' + full
                parts.push(
                    <a
                        key={key++}
                        href={href}
                        ...
                        className='text-blue-600 dark:text-blue-500 underline break-all'
                    >
                        {full}
                    </a>
                )
...
function getBasicMessageContent(message: IWebMessageInfo) {
    const { message: content } = message
    const type = messageType(content)
    if (type === 'text') return content.conversation
    else if (type === 'extendedText') return content.extendedTextMessage?.text
}
